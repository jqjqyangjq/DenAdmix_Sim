...
2:  all segragating sites between Den3, Den25 and 3 Neas      #included for final evaluation
3:  all segragating sites between Den3, and Altai, requiring near fixation in afrs. reflecting Archaic admixture.      #included for final evaluation
4:  all segragating sites between Den3 and 3 Neas. Archaic admixture like.  but not used in the end for sums
5.  all segragating sites between Den3, Den25 and 3 Neas. Archaic admixture like.       #included for final evaluation.   used for final evaluation with hmmix
...
rule admixfrog_call_missing:
    ...
    output:
        ...
        res = "sim/D_{divergence}/admixfrog_call_missing/{ref_type}/{pop}.ind_{ind}.res.xz",
        
    run:
        s = "admixfrog --infile {input.infile} --ref {input.ref} -o sim/D_{wildcards.divergence}/admixfrog_call_missing/"
        s += wildcards.ref_type
        s += "/{wildcards.pop}.ind_{wildcards.ind} --ll-tol 1e-3 --bin-size 2000 --ancestral anc --max-iter 200 --pos-mode --filter-pos 50 --filter-map 0"
        s += " --states "
...
rule den_prop:
    ...
    output:
        prop =  "sim/D_{divergence}/admixfrog_call_missing/{ref_type}/{pop}.ind_{ind}.DEN.sum",
    wildcard_constraints:
        ref_type = "3|5"
    run:
        s = "./Den_prop.py --state 'den' -n '{wildcards.ind}' --info '{wildcards.divergence} {wildcards.ref_type}' --out {output.prop} --input {input.rle} --length 20 "
        shell(s)
...
rule rerun_all_admixfrog_call:
    input:
        call_missing = expand("sim/D_{divergence}/admixfrog_call_missing/{ref_type}/{pop}.ind_{ind}.rle.xz", pop = ["ea"], ind=list(range(0,15)), ref_type = ['2','3','5'], divergence = divergence),
        
rule anno_missing_frog_call:
    input:
        call_missing = "sim/D_{divergence}/admixfrog_call_missing/{ref_type}/{pop}.ind_{ind}.rle.xz"
    output:
        call_missing = "sim/D_{divergence}/admixfrog_call_missing/{ref_type}/{pop}.ind_{ind}.rle.anno.xz"
    run:
        rle = pd.read_csv(input.call_missing)
...
rule sensi_all:
    input:
        ...
        expand("sim/D_{divergence}/admixfrog_call_missing/{ref_type}/ea.ind_{ind}.DEN.sum",
               divergence  = divergence, ref_type = [3,5], ind = list(range(0,15)))