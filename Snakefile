...
rule run_sim:
    ...
    run:
        ...
        sample_list = {
              ...
              "Intro_den_N": {"t_sample": [40], "ind": 1, "group": "DEN"},
              "ea": {"t_sample": [30, 30, 30, 30, 30, 15, 15, 15, 15, 15, 0, 0, 0, 0, 0], "ind": 1, "group": "EA"},
              "afr": {"t_sample": [0], "ind": 200, "group": "AFR"},
              "Intro_den_S": {"t_sample": [40], "ind": 1, "group": "DEN"}
          }
        samples = sim_demo.define_samples(sample_list)
        Demography= sim_demo.demo_archaic_introgression(N_p = 0.025, D_p = 0.004, D_s=int(wildcards.divergence))
        recomb_rate = 1.25e-8
        ...
        simulation_result = msprime.sim_mutations(simulation_result_tree, rate=1.25e-8,model=msprime.BinaryMutationModel())
...
rule get_true_segments:
    ...
    run:
        ...
        ts_help.get_introgressed_segments(ts,6,8,600000//29,file_name=output.TrueSeg_den_S,chrom=wildcards.chrom)
        ts_help.get_introgressed_segments(ts,6,2,600000//29,file_name=output.TrueSeg_nea_S,chrom=wildcards.chrom)
...
rule all_admixfrog_call:
    input:
        in_missing = expand("sim/D_{divergence}/admixfrog_call/{ref_type}/{pop}.ind_{ind}.rle.xz", pop = ["S_ea"], ind=list(range(0,15)), ref_type = ['1','2','3','4','5'], divergence = 390)
...
rule hmmix_call:
    ...
    run:
        s = "~/hmm_c/hmm_introgression/main.py gt_mode -data_type modern -count_file {input.infile} -mask_file mask_no_missing.chr1_20 "
...
rule hmmix_call_missing:
    ...
    wildcard_constraints:
        ind = "|".join([str(i) for i in list(range(0,100))])
    run:
        s = "~/hmm_c/hmm_introgression/main.py gt_mode -data_type modern -count_file {input.infile} -mask_file "
        s+= missing_mask
...
rule hmmix_call_hap:
    ...
    wildcard_constraints:
        ind = "|".join([str(i) for i in list(range(0,100))])
    run:
        s = "~/hmm_c/hmm_introgression/main.py gt_mode -data_type modern -count_file {input.infile} -mask_file mask_no_missing.chr1_20.hap "
